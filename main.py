...
vectorizer = pickle.load(open('Vectorizer_Tranform.pkl','rb'))
...
def get_suggestions():
    data = pd.read_csv('2023_and_prior.csv')
...
def populate_matches():
    ...
    #he data is being sent as JSON data, not form data. Therefore, you need to use request.get_data() to access the raw JSON data.
    movies_list = res['movies_list']

    movie_cards = {"https://image.tmdb.org/t/p/original"+movies_list[i]['poster_path'] if movies_list[i]['poster_path'] else "/static/movie_default.png": [movies_list[i]['title'],movies_list[i]['original_title'],movies_list[i]['vote_average'],datetime.strptime(movies_list[i]['release_date'], '%Y-%m-%d').year if movies_list[i]['release_date'] else "N/A", movies_list[i]['id']] for i in range(len(movies_list))}
...
def recommend():
    ...
    if(imdb_id != ""):
        ...
        return render_template('recommend.html',title=title,poster=poster,overview=overview,vote_average=vote_average,
            vote_count=vote_count,release_date=release_date,movie_rel_date=movie_rel_date,curr_date=curr_date,runtime=runtime,status=status,genres=genres,movie_cards=movie_cards,reviews="",casts=casts,cast_details=cast_details)
